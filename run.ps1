...
Import-Module .\scripts\ats.psm1 -Force
...
# TX hook (fallback, $Metrics не знайдено поруч):
# $Metrics = Use-TxInsideBacktest -Trades $Trades -Metrics $Metrics
...
try {
  ...
  # 1) спроба взяти угоди з уже існуючих змінних
  foreach ($n in @('Trades','allTrades','TradeList','Fills','Orders','Executions')) {
    try { $v = (Get-Variable -Name $n -ValueOnly -ErrorAction SilentlyContinue) } catch { $v = $null }
    if ($v) { $__tx_trades = $v; break }
  }

  # 2) якщо не знайшли — читаємо з останнього run-*/ (trades.csv | fills.csv | orders.csv)
  if (-not $__tx_trades) {
    $last = Get-ChildItem .\runs -Directory -Recurse |
            Where-Object Name -like 'run-*' |
            Sort-Object LastWriteTime -Desc | Select-Object -First 1
    if ($last) {
      foreach ($c in @('trades.csv','fills.csv','orders.csv')) {
        $hit = Get-ChildItem -Path $last.FullName -Recurse -Filter $c -File -ErrorAction SilentlyContinue |
               Sort-Object LastWriteTime -Desc | Select-Object -First 1
        if ($hit) { $__tx_trades = Import-Csv -Path $hit.FullName; break }
      }
    }
  }

  if ($__tx_trades) {
    $Metrics = Use-TxInsideBacktest -Trades $__tx_trades -Metrics $Metrics
  } else {
    Write-Host "[TX] Не знайшов жодної колекції угод — пропускаю Tx." -ForegroundColor Yellow
  }
}
catch {
  Write-Host "[TX] Помилка застосування Tx: $($_.Exception.Message)" -ForegroundColor Red
}
# === end TX hook ===
...
_tx_trades = Import-Csv -Path $hit.FullName; break }
      }
    }
  }

  # 3) якщо все одно нема — ставимо порожній список, щоб метрики отримали Tx-поля
  if (-not #requires -Version 7
param(
  [ValidateSet("G1","G2","G3","G4","G5","G6","All")]
  [string]$Gate = "All",
...
_tx_trades = @() }
  ...
  $Metrics = Use-TxInsideBacktest -Trades #requires -Version 7
param(
  [ValidateSet("G1","G2","G3","G4","G5","G6","All")]
  [string]$Gate = "All",
  [int]$CostBps
...
_tx_trades -Metrics $Metrics
}
catch {
  Write-Host "[TX] Помилка застосування Tx: $(#requires -Version 7
param(
  [ValidateSet("G1","G2","G3","G4","G5","G6","All")]
  [string]$Gate = "All",
  [int]$CostBps
