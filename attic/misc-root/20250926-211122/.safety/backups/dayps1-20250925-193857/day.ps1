...
# [shadow-live] hook — auto-check after paper run (task 9.2)
try {
$shadow = pwsh -NoProfile -File (Join-Path $PSScriptRoot 'scripts\Invoke-ShadowCheck.ps1') -Days 7 -ToleranceMinutes 15
Write-Host "[day] shadow-live check finished."
} catch {
Write-Warning "[day] shadow-live hook failed: $(# day.ps1 — v0.4 (strict idempotency, scheduler-DateTime, summary)
param(
[string]$Goal,
[datetime]$Date = (Get-Date).Date,
[switch]$NonInteractive,
...
# [shadow-live] hook — auto-check after paper run (task 9.2)
try {
\ = pwsh -NoProfile -File (Join-Path \ 'scripts\Invoke-ShadowCheck.ps1') -Days 7 -ToleranceMinutes 15
Write-Host "[day] shadow-live check finished."
} catch {
Write-Warning ("[day] shadow-live hook failed: {0}" -f \.Exception.Message)
}
.Exception.Message)"
}" -f \.Exception.Message)
}
