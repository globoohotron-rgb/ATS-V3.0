...
if (-not $PricesCsv) { $PricesCsv = Join-Path $root 'data\raw\ohlcv_sample.csv' }
if (-not $OutDir) {
...
function Get-SMA([double[]]$x,[int]$n){
  ...
  for($i=0;$i -lt $N;$i++){ $sum += $x[$i]; if($i -ge $n){ $sum -= $x[$i-$n] }
...
function Get-Std([double[]]$x){
  ...
  return [math]::Sqrt( ($x | ForEach-Object { ($_-$m)*($_-$m) } | Measure-Object -Sum).Sum / [math]::Max(1,($x.Length-1)) )
}
function Get-MaxDD([double[]]$rets){
  $N=$rets.Length; $eq = New-Object double[] ($N+1); $eq[0]=1.0
  for($i=0;$i -lt $N;$i++){ $eq[$i+1] = $eq[$i]*(1.0+$rets[$i]) }
  $peak=$eq[0]; $maxdd=0.0
  for($i=1;$i -lt $eq.Length;$i++){ if($eq[$i] -gt $peak){ $peak=$eq[$i] }
    $dd = ($eq[$i]/$peak)-1.0; if($dd -lt $maxdd){ $maxdd=$dd } }
  return -$maxdd
}
function Get-Pos([double[]]$px,[int]$f,[int]$s){
  $smaF=Get-SMA $px $f; $smaS=Get-SMA $px $s; $N=$px.Length; $pos=New-Object double[] $N
  for($i=0;$i -lt $N;$i++){
...
function Apply-Costs([double[]]$rets,[double[]]$pos,$COSTS){
  $N=$rets.Length; $out=New-Object double[] $N
...
function Get-Metrics([double[]]$series){
  $ann=252.0; $mu=($series|Measure-Object -Average).Average; $sd=Get-Std $series
  $sh= if($sd -gt 0){ ($mu/[double]$sd)*[math]::Sqrt($ann) } else { 0.0 }
  [PSCustomObject]@{ PnL=($series|Measure-Object -Sum).Sum; Sharpe=$sh; MaxDD=(Get-MaxDD $series) }
}
...
$rets = New-Object double[] $px.Count; for($i=1;$i -lt $px.Count;$i++){ $rets[$i]=($px[$i]/$px[$i-1])-1.0 }; $rets[0]=0.0

# --- grid (≤8)
$fastSet=@(5,10); $slowSet=@(20,50)
$grid=@(); foreach($f in $fastSet){ foreach($s in $slowSet){ if($f -lt $s){ $grid+=[pscustomobject]@{Fast=$f;Slow=$s} } } }
if($grid.Count -gt 8){ throw "Grid too large: $($grid.Count) > 8 (guardrail)" }

# --- WF
$start=$WF.TrainBars
if($WF.Align -eq 'MonthStart'){ for($j=$start;$j -lt $px.Count;$j++){ if($ts[$j].Date.Day -le 3){ $start=$j; break } } }
$windows=@(); $aggOOS=@(); $aggBH=@(); $aggIS=@()

while(($start + $WF.OOSBars) -le $px.Count){
  $isBeg=$start-$WF.TrainBars; $isEnd=$start-1; $oBeg=$start; $oEnd=$start+$WF.OOSBars-1

  # select best by IS Sharpe
  $scores=@()
  for($g=0;$g -lt $grid.Count;$g++){
    $p=$grid[$g]; $pos=Get-Pos $px $p.Fast $p.Slow; $str=Apply-Costs $rets $pos $COSTS
    $isSlice=$str[$isBeg..$isEnd]; $scores+=[pscustomobject]@{ G=$g; M=(Get-Metrics $isSlice); Slice=$isSlice } }
  $best = $scores | Sort-Object { -$_.M.Sharpe } | Select-Object -First 1
  $gIdx=$best.G; $mIS=$best.M; $isSliceBest = $best.Slice

  # OOS for best + neighbors
  $nbr=@($gIdx); for($k=1;$k -le [int]$WF.Neighbors;$k++){ if($gIdx-$k -ge 0){$nbr+=$gIdx-$k}; if($gIdx+$k -lt $grid.Count){$nbr+=$gIdx+$k} }; $nbr=$nbr|Sort-Object -Unique
  $oosBestRet=@(); $recN=@()
  foreach($gi in $nbr){
    $p=$grid[$gi]; $pos=Get-Pos $px $p.Fast $p.Slow; $str=Apply-Costs $rets $pos $COSTS
    $oosSlice=$str[$oBeg..$oEnd]; $m=Get-Metrics $oosSlice
  ...
  $mBest=Get-Metrics $oosBestRet; $bhSlice=$rets[$oBeg..$oEnd]; $mBH=Get-Metrics $bhSlice

  $windows += [pscustomobject]@{
    IS_Beg=$ts[$isBeg].Date; IS_End=$ts[$isEnd].Date; OOS_Beg=$ts[$oBeg].Date; OOS_End=$ts[$oEnd].Date;
    ...
    IS  =[pscustomobject]@{ PnL=$mIS.PnL; Sharpe=$mIS.Sharpe; MaxDD=$mIS.MaxDD };
    Neigh=$recN; Bench=[pscustomobject]@{ PnL=$mBH.PnL; Sharpe=$mBH.Sharpe; MaxDD=$mBH.MaxDD }
  }

  $aggOOS += $oosBestRet; $aggBH += $bhSlice; $aggIS += $isSliceBest
  $start += [int]$WF.StepBars
}

$aggBest=Get-Metrics $aggOOS; $aggBHm=Get-Metrics $aggBH; $aggISm=Get-Metrics $aggIS
New-Item -ItemType Directory -Path $OutDir -Force | Out-Null
$csvPath=Join-Path $OutDir 'wf.oos.csv'; $jsonPath=Join-Path $OutDir 'wf.results.json'
...
$rowsOut = foreach($w in $windows){
  [pscustomobject]@{
    ...
    IS_PnL=$w.IS.PnL; IS_Sharpe=$w.IS.Sharpe; IS_MaxDD=$w.IS.MaxDD;
...
$meta = [pscustomobject]@{
  ...
  Grid=$grid
  Windows=$windows
  Aggregate=[pscustomobject]@{ Best=$aggBest; IS=$aggISm; BH=$aggBHm }
}
